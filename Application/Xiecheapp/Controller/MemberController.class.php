...
class MemberController extends CommonController {
    ...
    public function save() {
		...
		if ($_POST['uid']) { //保存成功
			...
			if (cookie('_currentUrl_') == '/index.php/member/complete_member'){
	...
	*/
	...
	public function _before_insert(){
		...
		if (empty($_POST['repassword']) and $_SERVER['HTTP_REFERER']==WEB_ROOT.'/member/add'){
		    $this->error('确认密码不能为空！');
		}
	    if ($_POST['password'] !=$_POST['repassword'] and $_SERVER['HTTP_REFERER']==WEB_ROOT.'/member/add'){
		...
		*/
		...
		if(!is_numeric($_POST['mobile']) or strlen($_POST['mobile'])!=11){
		    $this->error('手机号码错误！');
		}
	    if ($_POST['mobile'] and !eregi("^1[0-9]{10}$",$_POST['mobile'])){
		    $this->error('手机号码格式错误！');
