...
class Message {
	public function Message($biz_content) {
		...
		if ($MsgType == "text") {
			...
			writeLog ( "收到的文本：" . $text );
			if($text == "回复测试"){
				$text_msg = $push->mkTextMsg ( "你好，这是对话消息<br/>".$FromUserId."<br/>");
		...
		if ($EventType == "follow") {
			...
			$url = 'http://www.xieche.com.cn/alipay/followadd';
			$post = array('FromUserId'=>$FromUserId);
			$this->curl_file_get_contents($url,$post);

		} elseif ($EventType == "unfollow") {
			// 处理取消关注消息
			$url = 'http://www.xieche.com.cn/alipay/unfollow';
			$post = array('FromUserId'=>$FromUserId);
			$this->curl_file_get_contents($url,$post);
