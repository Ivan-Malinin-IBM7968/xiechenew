...
function get_data_from_server($address, $service_port, $send_data) {
	...
	$now = time();
	$sql = "INSERT INTO xc_ccbpaylog SET orderid='1',payment='1',logs='".$socket."',create_time='".$now."'";
	//echo $sql;exit;
	$mysql->query($sql);

	if ($socket < 0) {
		echo "socket����ʧ��ԭ��: " . socket_strerror($socket) . "\n";
	} else {
		echo "socket_create OK��HE HE.\n";
	}
	$result = socket_connect($socket, $address, $service_port);
	$now = time();
	$sql = "INSERT INTO xc_ccbpaylog SET orderid='1',payment='1',logs='".$result."',create_time='".$now."'";
	//echo $sql;exit;
	$mysql->query($sql);
	if ($result < 0) {
		echo "SOCKET����ʧ��ԭ��: ($result) " . socket_strerror($result) . "\n";
	} else {
		echo "OK.\n";
	}
	//��������
	...
	$now = time();
	$sql = "INSERT INTO xc_ccbpaylog SET orderid='2',payment='2',logs='".$write."',create_time='".$now."'";
	//echo $sql;exit;
	$mysql->query($sql);
	...
	while ($out = socket_read($socket, 20)) {
		$now = time();
		$sql = "INSERT INTO xc_ccbpaylog SET orderid='5',payment='2',logs='".$out."',create_time='".$now."'";
		//echo $sql;exit;
		$mysql->query($sql);
		echo $out;
	}
...
//ccb支付的log
function saveccblog($orderid,$payment,$logs){
	global $mysql;
	$now = time();
	$sql = "INSERT INTO xc_ccbpaylog SET orderid='".$orderid."',payment='".$payment."',logs='".$logs."',create_time='".$now."'";
	$mysql->query($sql);
}
//获取上门保养的订单
function get_reservation_order($reservation_id){
	global $mysql;
	//$res_order_id = get_orderid($reservation_id);
	$sql = "select * from xc_reservation_order where id='".$reservation_id."' ";
	$reservation=$mysql->query($sql,'assoc');
...
//更改上门保养的订单的状态
function update_reservation_order_state($order_id,$pay_status,$pay_type){
	global $mysql;
	//$order_id = get_orderid($order_id);
	//$rand_str = randString(9,1);
	//$now = time();
	$sql = "UPDATE xc_reservation_order SET pay_type='".$pay_type."',pay_status='".$pay_status."' WHERE id='".$order_id."' LIMIT 1";
	$mysql->query($sql);
	//return $rand_str;
}

function save_txmembercoupon_pay($membercoupon_ids,$trade_no,$gmt_payment,$total_fee=0,$trade_status=''){
	global $mysql;
	$now = time();
	$sql = "INSERT INTO xc_commcouponpay SET membercoupon_ids='".$membercoupon_ids."',trade_no='".$trade_no."',gmt_payment='".$gmt_payment."',total_fee='".$total_fee."',trade_status='".$trade_status."',create_time='".$now."'";
	//echo $sql;exit;
	$mysql->query($sql);
}
...
//短信接口
function curl_sms($post='',$charset=null,$SMS_METHOD){
	global $mysql;
	...
	if($SMS_METHOD == 1) {
		...
		$now = time();
		$sql = "INSERT INTO xc_ccbpaylog SET orderid='sms',payment='',logs='".$post['content']."',create_time='".$now."'";
		$mysql->query($sql);
	}else{
		$now = time();
		$sql = "INSERT INTO xc_ccbpaylog SET orderid='sms3',payment='',logs='".$post.'|'.$SMS_METHOD."',create_time='".$now."'";
		$mysql->query($sql);
...
function update_membercoupon_state($membercoupon_id,$pay_status){
	...
	$now = time();
	$sql = "UPDATE xc_membercoupon SET pay_type=2,is_pay='".$pay_status."',pay_time='".$now."',coupon_code='".$rand_str."' WHERE membercoupon_id='".$membercoupon_id."' LIMIT 1";
	$mysql->query($sql);
}
