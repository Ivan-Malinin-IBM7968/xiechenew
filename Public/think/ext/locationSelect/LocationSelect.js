...
com.elfvision.DEBUG = false;
 ...
 (function() {
    ...
    getJson = function(config) {
        ...
        request.open("GET", config.url, true);//去掉了随机
    ...
    ListView.prototype.rebuildList = function(e) {
        ...
        function(item, index) {
            opt = new Option();
            //设置值
            opt.setAttribute("value", item.text);
...
//初始化列表
	...
	*/
    LocationSelect = function(config) {
        ...
        this.detectGeoLocation = config.detectGeoLocation === undefined ? true: config.detectGeoLocation;//默认自动检测地区
        this.detector = config.detector || GeoDetect;//检测地区函数
...
it's worthwhile.	
        ...
        function() {
            ...
            function(data) {
                ...
                if (data.query.results.json.status === "OK") {
					...
					var hid_prov = $("#hid_prov").val();
					var hid_city = $("#hid_city").val();
					var hid_area = $("#hid_area").val();
					if (hid_prov == ""){
						that.select([geo.province, geo.city]);
					}else{
						that.select([hid_prov , hid_city , hid_area]);
					}
                }
            });
	...
	*/
    ...
    //import to the my namespace 提供给外部访问
...
if(window.jQuery !== undefined) {
	...
	$.LocationSelect = {
		build : function(user_config) {
			...
			instance = new com.elfvision.kit.LocationSelect(config);
