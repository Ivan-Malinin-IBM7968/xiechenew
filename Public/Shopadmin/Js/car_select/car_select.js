$(document).ready(function(){
	...
	$(".checkbox_brand").click(function(){
		var brand_ids="";
		$("input[name='checkbox_brand_id']:checkbox:checked").each(function(){
			if(brand_ids==""){
				brand_ids = $(this).val();
			}else{
				brand_ids+=','+$(this).val();
		...
		//var url = "__URL__/carselect/ajax_get_brand";
		...
		$.ajax({
			type: "POST",
			url: "/index.php/carselect/ajax_get_brand",
			cache: false,
			...
			success: function(data){
				if (data!='null'){
					...
					for (i=0; i<param.length; i++ ){
						...
						$("#get_brand").append("<option value='"+param[i]['brand_id']+"' >"+param[i]['brand_name']+"</option>");
					}
				}
			}
		})
	...
	$("#get_brand").change(function(){
		...
		if($(this).val() != '0'){
			$.ajax({
				...
				url: "/index.php/carselect/ajax_get_carinfo",
				cache: false,
				data: "brand_id="+$(this).val()+"brand_ids="+brand_ids,
				success: function(data){
					if (data!='null'){
						var param = eval(data);
						$("#get_series").append("<option value='0'>请选择车系</option>");
