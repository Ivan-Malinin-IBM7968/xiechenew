...
function sortBy (field,sort,path){
	location.href = URL+"/"+path+"/_order/"+field+"/_sort/"+sort;
}
function cache(){
...
function memo(id){
	document.getElementById('memotextarea_'+id).style.display='';
	document.getElementById('memotext_'+id).style.display='none';
	
}
function savememo(id){
	var memo = document.getElementById('memo_'+id).value;
	if (memo == '')
	{
		ThinkAjax.send(URL+'/savememo/id/'+id,'ajax=1');
	}else{
		ThinkAjax.send(URL+'/savememo','ajax=1&memo='+memo+'&id='+id);
	}
}
function resetpw(id){
	ThinkAjax.send(URL+'/resetpw/id/'+id,'ajax=1');
}
function recycle(id){
	var keyValue;
	if (id)
...
function shenhe(id){
	location.href = URL+"/shenhe/id/"+id;
}
function output(){
...
function add_coupon(id){
	if (id)
	{
		 location.href  = URL+"/add_coupon/id/"+id;
	}else{
		 location.href  = URL+"/add_coupon/";
	}
}
function coupon(id){
	var keyValue;
	...
	location.href =  URL+"/coupon/id/"+keyValue;
}
function edit(id){
	var keyValue;
	if (id)
...
function shenhe(id){
	...
	if (window.confirm('确实要审核通过选择项吗？'))
	{
		//ThinkAjax.send(URL+"/shenhe/","id="+keyValue+'&ajax=1',doShenhe);
		location.href =  URL+"/shenhe/id/"+keyValue;
	}
}
...
function doDelete(data,status){
		...
		{
            var table = $('checkList');
            var ck = document.getElementsByName('key');
            var rowIndex;
