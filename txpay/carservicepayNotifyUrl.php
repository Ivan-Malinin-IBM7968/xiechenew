...
if($resHandler->isTenpaySign()) {
	...
	if($httpClient->call()) {
		...
		if($queryRes->isTenpaySign() && $queryRes->getParameter("retcode") == "0" && $queryRes->getParameter("trade_state") == "0" && $queryRes->getParameter("trade_mode") == "1" ) {
			...
			//财务通用户号
			$buyer_id = $queryRes->getParameter("buyer_id");
			//订单创建时间
			...
			$order_info = get_orderinfo($out_trade_no);
			save_txmembercoupon_pay($out_trade_no,$transaction_id,$buyer_id,$gmt_create,$gmt_payment,$total_fee,$queryRes->getParameter("trade_state"),0);
