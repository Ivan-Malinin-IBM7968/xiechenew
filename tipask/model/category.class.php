...
class categorymodel {
    ...
    function list_by_grade($grade=1) {
        $categorylist = array();
        $query = $this->db->query("select id,name,questions,grade from " . DB_TABLEPRE . "category where grade=1 order by displayorder asc,id asc limit 5");
        while ($category1 = $this->db->fetch_array($query)) {
            $query2 = $this->db->query("select id,name,questions from " . DB_TABLEPRE . "category where pid=$category1[id] and grade=2 order by displayorder asc,id asc limit 4");
	...
	function get_cid_by_series_id($series_id) {
        $category = $this->db->fetch_first("SELECT id FROM " . DB_TABLEPRE . "category WHERE series_id='$series_id'");
		return $category['id'];
    }
	
	function get_cid_by_brand_id($brand_id) {
        $category = $this->db->fetch_first("SELECT id FROM " . DB_TABLEPRE . "category WHERE brand_id='$brand_id' AND series_id='0'");
		return $category['id'];
    }
