...
class usermodel {
    ...
    function get_by_uid($uid, $loginstatus=1) {
		...
		$expert = $this->db->fetch_first("SELECT shop_id FROM " . DB_TABLEPRE . "expert WHERE uid='$uid'");
    ...
    function uppass($uid, $password) {
		...
		$this->db->select_db("tp_xieche");
		$sql = "UPDATE  xc_member SET `password`='{$password}' WHERE `uid`='{$uid}'  LIMIT 1";
	...
	function get_shop_info($shop_id) {
		$this->db->select_db("tp_xieche");
		$shop_info = $this->db->fetch_first("SELECT * FROM xc_shop WHERE id='$shop_id' ");

		$query = $this->db->query("SELECT * FROM xc_shop_fs_relation WHERE shopid='$shop_id' ");
        while ($shop_fs_relation = $this->db->fetch_array($query)) {
            $tmp['fsid'] = $shop_fs_relation['fsid'];
            $relation[] = $tmp;
        }
        ...
        if ($relation){
            foreach ($relation as $k=>$v){
                $query = $this->db->query("SELECT * FROM xc_carseries WHERE fsid='$v[fsid]' ");
				while ($xc_carseries = $this->db->fetch_array($query)) {
					$brand_id_arr[$xc_carseries['brand_id']] = $xc_carseries['brand_id'];		
				}
            }
        }
		$brands_id = implode(',',$brand_id_arr);
		$query = $this->db->query("SELECT brand_name FROM xc_carbrand WHERE brand_id IN ( ".$brands_id." ) ");
		while ($brands_name = $this->db->fetch_array($query)) {
			$brands_tmp['brand_name'] = $brands_name['brand_name'];	
			$brands_result[] = $brands_tmp;
		}
		$shop_info['brands'] = $brands_result;
		return $shop_info;
	}

	//xieche库获取预约超时折扣
	function get_shop_timesale($shop_id) {
		$this->db->select_db("tp_xieche");
		$list_timesale = $this->db->fetch_all("SELECT * FROM xc_timesale WHERE shop_id='$shop_id' AND status=1 ");
		if ($list_timesale){
		    foreach ($list_timesale as $_k=>$_v){
		        if ($_v['week']){
		            $week_name = explode(',',$_v['week']);
        			foreach($week_name as $vv){
        			    if (trim($vv)=='0'){
        			        $vv = '日';
        			    }
					
        			    $list_timesale[$_k]['week_name_s'] .= '周'.$vv.',';
        			}
		        }
		        if ($list_timesale[$_k]['week_name_s']){
		            $list_timesale[$_k]['week_name_s'] = mb_substr($list_timesale[$_k]['week_name_s'],0,-1);
		        }
				$now = time();

		        $list_timesale[$_k]['timesaleversion'] = $this->db->fetch_all("SELECT * FROM xc_timesaleversion WHERE timesale_id='$_v[id]' AND status=1 AND s_time<$now AND e_time>$now");
