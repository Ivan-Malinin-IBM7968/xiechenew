...
class questionmodel {
    ...
    var $ordertable = array(
        ...
        '1' => ' AND status=1  AND price=0 ORDER BY time DESC',
    ...
    function list_by_condition($condition, $start=0, $limit=10) {
        $questionlist = array();
        $query = $this->db->query("SELECT * FROM `" . DB_TABLEPRE . "question` WHERE $condition limit $start , $limit");
    ...
    function expertanswer($condition, $start=0, $limit=10) {
		//获取专家账号
		$questionlist = array();
		$expert_query = $this->db->query("SELECT * FROM `" . DB_TABLEPRE . "user` WHERE expert='1' ");
		while ($expert = $this->db->fetch_array($expert_query)) {
			$uid = $expert['uid'];
			$username = $expert['username'];
			$query = $this->db->query("SELECT * FROM `" . DB_TABLEPRE . "answer` WHERE authorid='".$uid."'");
			while ($answer = $this->db->fetch_array($query)) {
				$qid = $answer['qid'];
				$questionquery = $this->db->query("SELECT * FROM `" . DB_TABLEPRE . "question` WHERE id=$qid $condition ");
				$question = $this->db->fetch_array($questionquery);
    ...
    function rownum_by_time($uid, $hours=1) {
        ...
        return $this->db->fetch_total('question', " `time`>$starttime AND `time`<$endtime AND authorid=$uid");
    }

    /* 问题列表，根据指定的分类名称，和分类id，以及status来查询 */

    function list_by_cfield_cvalue_status($cfield='cid1', $cvalue=0, $status=0, $start=0, $limit=10,$answer=0) {
        $questionlist = array();
        $sql = "SELECT * FROM " . DB_TABLEPRE . "question WHERE 1=1 ";
