    ...
    function fromcache($cachename, $cachetime=3) {
        ...
        switch ($cachename) {
            ...
            case 'rewardlist'://悬赏的问题
                ...
                break;
			case 'alllist_answer': //所有问题，网友正在问
                $this->load('question');
                $cachedata = $_ENV['question']->list_by_cfield_cvalue_status('', 0, 'all', 0, $this->setting['list_indexnosolve'],1);
                break;
			case 'nosolvelist_answer': //待解决问题，网友正在问
                $this->load('question');
                $cachedata = $_ENV['question']->list_by_cfield_cvalue_status('', 0, 1, 0, $this->setting['list_indexnosolve'],1);
                break;
            case 'solvelist_answer'://已解决问题
                $this->load('question');
                $cachedata = $_ENV['question']->list_by_cfield_cvalue_status('', 0, 2, 0, $this->setting['list_indexnosolve'],1);
                break;
            case 'rewardlist_answer'://悬赏的问题
                $this->load('question');
                $cachedata = $_ENV['question']->list_by_cfield_cvalue_status('', 0, 4, 0, $this->setting['list_indexreward'],1);
                break;
            ...
            case 'categorylist'://首页左侧分类列表
                ...
                break;
			case 'newest_question'://最新问题
                $this->load('question');
                $cachedata = $_ENV['question']->list_by_condition("1=1 ORDER BY time DESC ");
                break;
            case 'recommendlist'://精彩推荐问题
                $this->load('question');
                $cachedata = $_ENV['question']->list_by_condition("`status`=6 ORDER BY time DESC ");
                break;
            ...
            case 'expertlist':
                ...
                break;
			case 'expertanswerlist'://专家精彩答复问题
                $this->load('question');
                $cachedata = $_ENV['question']->expertanswer("ORDER BY time DESC ");
                break;
            case 'topiclist':
                $this->load('topic');
			...
			case 'hottag':
                $this->load("tag");
                $cachedata = $_ENV['tag']->get_list();
                break;
			case 'expert_answer':
                $this->load("expert");
                $cachedata = $_ENV['expert']->get_list_by_order('', 'answers');
                break;
			case 'expert_credit1':
                $this->load("expert");
                $cachedata = $_ENV['expert']->get_list_by_order('', 'credit1');
                break;
			case 'expertlsit':
                $this->load("expert");
                $cachedata = $_ENV['expert']->get_expertlist();
                break;
        }
...
//判断是否需要升级
    ...
    function checkable($url) {
        ...
        if (1 == $this->user['groupid'])
            return true;
        $regulars = explode(',', 'user/space,user/login,user/logout,user/code,user/getpass,user/resetpass,index/help,js/view,attach/upload,' . $this->user['regulars']);
