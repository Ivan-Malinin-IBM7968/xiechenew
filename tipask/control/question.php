...
class questioncontrol extends base {
    ...
    function onadd() {
        ...
        if (isset($this->post['submit'])) {
            ...
            /* 检查提问数是否超过组设置 */
            ($this->user['questionlimits'] && ($_ENV['question']->rownum_by_time($this->user['uid']) >= $this->user['questionlimits'])) &&
            ...
            if ($askfromuid) {
                ...
                $this->sendmail($touser, '问题求助:' . $title, $description . '<br /> <a href="' . SITE_URL . $this->setting['seo_prefix'] . $viewurl . $this->setting['seo_suffix'] . '">点击查看问题</a>');
            }
    ...
    function onanswer() {
        ...
        /* 检查提问数是否超过组设置 */
        ($this->user['answerlimits'] && ($_ENV['answer']->rownum_by_time($this->user['uid']) >= $this->user['answerlimits'])) &&
