...
class categorycontrol extends base {
	...
	function onajaxgetcidbyseriesid() {
		$series_id = $this->post['series_id'];
        $cid = $_ENV['category']->get_cid_by_series_id($series_id);
		echo $cid;exit;
    }

	function onajaxgetcidbybrandid() {
		$brand_id = $this->post['brand_id'];
        $cid = $_ENV['category']->get_cid_by_brand_id($brand_id);
		echo $cid;exit;
    }
