...
<div style="width:100%; height:15px;color:#000;margin:0px 0px 10px;">
    ...
    <table width="100%" cellspacing="1" cellpadding="4" align="center" class="tableborder">
        <tbody>
            ...
            <tr>
                <td colspan="3" rowspan="2" class="altbg2">回答时间<input type="text" onclick="showcalendar();" name="srchdatestart" value="<?=$srchdatestart?>" />到&nbsp;<input class="txt" onclick="showcalendar();" name="srchdateend" value="<?=$srchdateend?>"/></td>
            </tr>
        </tbody>
    </table>
</form>
...
<form name="answerlist" method="post">
    <table width="100%" border="0" cellpadding="4" cellspacing="1" class="tableborder">
        <tr class="header">
            <td width="7%"><input class="checkbox" id="chkall" onclick="checkall('aid[]')" type="checkbox" name="chkall"><label for="chkall">全选</label></td>
            <td  width="40%">回答内容</td>
            <td  width="10%">回答人</td>
            <td  width="13%">回答时间</td>
            <td  width="14%">IP</td>
...
<? if(is_array($answerlist)) { foreach($answerlist as $answer) { ?>
        <tr>
            ...
            <td class="altbg2"><? if($answer['adopttime']) { ?><img title="已被采纳为最佳答案" src="<?=SITE_URL?>css/common/icn_2.gif"><? } else { ?>否<? } ?></td>
        </tr>
