...
<tbody><tr class="header"><td>分类列表&nbsp;&nbsp;&nbsp;<input type="button" style="cursor:pointer" onclick="document.location.href='index.php?admin_category/add/new<?=$setting['seo_suffix']?>'" value="添加新分类" /></td></tr>
<tr class="altbg1"><td>分类的排序可以通过鼠标拖动来改变，当鼠标在某一分类上面时，按住左键即可上下移动。</td></tr>
<tr class="altbg2"><td>
<a href="index.php?admin_category/default<?=$setting['seo_suffix']?>">根目录</a><? if(isset($navlist)) { if(is_array($navlist)) { foreach($navlist as $nav) { ?>
...
<td width="10%"><input class="checkbox" id="chkall" onclick="checkall('cid[]')" type="checkbox" name="chkall"><label for="chkall">删除</label></td>
<td width="30%">分类名称</td>
...
<td width="30%" class="altbg2"><a href="index.php?admin_category/view/<?=$cate['id']?><?=$setting['seo_suffix']?>"><input name="order[]" type="hidden" value="<?=$cate['id']?>"/><strong><?=$cate['name']?></strong></a></td>
<td width="10%" class="altbg2" align="center"><img src="<?=SITE_URL?>css/common/admin/view.png" style="cursor:pointer" onclick="document.location.href='index.php?admin_category/view/<?=$cate['id']?><?=$setting['seo_suffix']?>'"></td>
<td width="10%" class="altbg2" align="center"><img src="<?=SITE_URL?>css/common/admin/add.png" style="cursor:pointer" onclick="document.location.href='index.php?admin_category/add/<?=$cate['id']?>/<?=$cate['pid']?><?=$setting['seo_suffix']?>'"></td>
<td width="10%" class="altbg2" align="center"><img src="<?=SITE_URL?>css/common/admin/edit.png" style="cursor:pointer" onclick="document.location.href='index.php?admin_category/edit/<?=$cate['id']?><?=$setting['seo_suffix']?>'"></td>
</tr>
</table>
</li>
<? } } ?>
