...
<? } } ?>
                        ...
                        <? } ?>                        <div class="xs">
                            ...
                            <span class="twsj">
                                <img src="<?=SITE_URL?>css/default/lefttime.gif"> 解决时间：<?=$bestanswer['adopttime']?>&nbsp;&nbsp; 
        ...
        <div class="zjda">
            <div class="zjdatitle">
                <ul>
                    <li class="zja1"></li>
                    <li class="zja2">
                        <div class="fimg"><img width="44px" height="41px" align="absmiddle" src="<?=SITE_URL?>css/default/bestanswer.gif"></div>
                    </li>
                    <li class="zja5">
                        最佳答案
                    </li>
            ...
            <div class="zjallcont">
                <div class="zjleft">
                    ...
                    <div class="touxiang"><a target="_blank" href="<?=SITE_URL?>?u-<?=$bestanswer['authorid']?>.html"><img align="absmiddle" src="<?=$bestanswer['author_avartar']?>" onmouseout="closeuserinfo();" onmouseover="showuserinfo('<?=$bestanswer['authorid']?>');"></a></div>
                    <div class="hyxinxi"> <span class="name"><a target="_blank" href="/jkzd_showperson.html?id=8702"><?=$bestanswer['author']?></a></span></div>
                </div>
                <div class="zjright">
                    <div class="zjt">
                        <img align="absmiddle" src="<?=SITE_URL?>css/default/lefttime.gif" /> 回答于:<?=$bestanswer['time']?>&nbsp;&nbsp;<a href="javascript:inform('<?=$bestanswer['author']?>',2);">举报</a>
                    </div>	
                    <div class="qfgx"></div>
                    <div class="zjcont">
...
<? if(is_array($bestanswer['tags'])) { foreach($bestanswer['tags'] as $tagindex => $tag) { ?>
...
<? } } ?>
                        <div class="clr"></div>
                        <div id="pinglun">
                            <? if($bestanswer['comment']) { ?>                            <h4>提问者对最佳答案的评价：</h4>
                            <?=$bestanswer['comment']?><? } else { ?>谢谢您的解答！
                            <? } ?>                            <? if($bestanswer['commentlist']) { ?>                            <fieldset>
                                <legend><a title="查看评分记录" href="<?=SITE_URL?>?question/ajaxanswercomment/<?=$bestanswer['id']?>.html" >该回答评分记录</a></legend>
                                <ul>
                                    
<? if(is_array($bestanswer['commentlist'])) { foreach($bestanswer['commentlist'] as $comment) { ?>
...
<? } } ?>
                    ...
                    <? if(1==$user['groupid']) { ?>                    <div  class="wlhdb">
                        <h1>管理该回答:</h1>
                        <span class="questioncontrol"><input type="button" value="编辑内容" class="button1" onclick="javascript:document.location='<?=SITE_URL?>?question/editanswer/<?=$bestanswer['id']?>.html'"/></span>
                        <div class="clr"></div>
                    </div>
                    <? } ?>                    <div class="zjlhdb ">
                        <p class="zjlhdbimg">
                            <img width="35px" height="36px"  style="cursor:pointer" alt="支持" onclick="add_comment(1,<?=$bestanswer['id']?>,<?=$bestanswer['authorid']?>);" src="<?=SITE_URL?>css/default/agree.gif">&nbsp;
                            <img width="35px" height="36px"  style="cursor:pointer" alt="反对" onclick="add_comment(0,<?=$bestanswer['id']?>,<?=$bestanswer['authorid']?>);" src="<?=SITE_URL?>css/default/aganist.gif">&nbsp;
                        </p>
...
<? if(is_array($solvelist)) { foreach($solvelist as $solve) { ?>
                        <? if($question['id'] != $solve['id']) { ?>                        <li><a title="<?=$solve['title']?>" href="<?=SITE_URL?>?q-<?=$solve['id']?>.html" target="_blank"><?=$solve['title']?></a>&nbsp;<span class="lei">(<?=$solve['answers']?>个回答)</span></li>
                        <? } ?>                        
<? } } ?>
                    </ul>
                </div>
...
<? if(is_array($answerlist)) { foreach($answerlist as $index => $answer) { ?>
            <? if(!$answer['adopttime']) { ?>            <? $index++ ?>            <div class="qtallcont">
                <div class="qtleft">
...
<? if(is_array($answer['tags'])) { foreach($answer['tags'] as $tagindex => $tag) { ?>
                        <? if(($tagindex%2) != 0) { ?>                        <div class="ask_gai"></div>
                        <div class="ask_zhuiwen">
                            <span style="color:#5EBB0B;float:left;margin-right:5px;">追问:</span><?=$tag?>
                        </div>                        
                        <? } else { ?>                        <? if($tagindex!=0) { ?>                        <div class="ask_gai"></div>
                        <? } ?>                        <div class="ask_zhuiwen">
                            <span style="color:#999999;float:left;margin-right:5px;">回答:</span><?=$tag?>
                        </div>
...
<? } } ?>
                        ...
                        <? } ?>                        <? if(1==$user['groupid']) { ?>                        <div class="clr"></div>
                        <div  class="wlhdb">
                            <h1>管理该回答:</h1>
                            <span class="questioncontrol"><input type="button" value="编辑内容" class="button1" onclick="javascript:document.location='<?=SITE_URL?>?question/editanswer/<?=$bestanswer['id']?>.html'"/></span>
...
<script type="text/javascript">
    ...
    function adoptanswer(aid){
        $.dialog({
            ...
            content:'对该回答的感言： <div><form name="editanswerForm"  action="<?=SITE_URL?>?question/adopt.html" method="post" ><textarea id="mc" name="content" style="width: 99%; padding-top: 1px; font-size: 14px;" rows="4" cols="50" >谢谢您的解答！</textarea><input type="hidden"  value="<?=$question['id']?>" name="qid"/><input type="hidden" id="ma" value="'+aid+'" name="aid"/></form></div>'
        });
    }
    //追问模块--追问
    ...
    function taganswer(aid){
        $.dialog({
            ...
            content:'继续回答的内容： <div><form name="tagAskForm"  action="<?=SITE_URL?>?question/tagask.html" method="post" ><textarea id="mc" name="tag_answer" style="width: 99%; padding-top: 1px; font-size: 14px;" rows="4" cols="50" ></textarea><input type="hidden"  value="<?=$question['id']?>" name="qid"/><input type="hidden" id="ma" value="'+aid+'" name="aid"/></form></div>'
        });
    }
