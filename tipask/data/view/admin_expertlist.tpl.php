...
<div style="width:100%; height:15px;color:#000;margin:0px 0px 10px;">
    ...
    <table width="100%" cellspacing="1" cellpadding="4" align="center" class="tableborder">
        <tbody>
            <tr class="header" ><td colspan="4">专家管理</td></tr>
            <tr class="altbg1"><td colspan="4">1、用户名必须是系统已注册用户。2、分类为系统已添加分类，多个分类用,隔开,最多不超过3个</td></tr>
            <tr>
                <td width="20%" >用户名:<input type="text" name="username"/></td>
                <td  width="30%">擅长分类ID:<input type="text" name="goodatcategory" id="categorys" size="50" onfocus="showselect()"/></td>
                <td width="30%" >店铺:<span><select name="shop_id" id="shoplist"></select></span></td>
...
<? } } ?>
        <tr class="altbg1"><td colspan="3" class="altbg1"  align="left"><input type="submit" name="submit"  value="提&nbsp;交" /></td></tr>
    </table>
</form>
<br>
...
function get_shoplist(cids){
$.ajax({
            type: "POST",
            url: "../index.php/shop/getshoplist",
            data: "cid="+cids,
            success: function(data){
$("#shoplist").html("");
if (data!='0'){
$("#shoplist").append("<option value='0'>请选择店铺</option>");
var param = eval(data);
//alert(data);
for (i=0; i<param.length; i++ ){
$("#shoplist").append("<option value='"+param[i]['id']+"'>"+param[i]['shop_name']+"</option>");
}
}
            }
        });    
    ...
    function showselect(){
        ...
        $.dialog({
            ...
            fnOk:function(){ 
                var cids='';
                $("ul li input[name='cids[]']").each(function(){
                    cids +=$(this).val()+' ';
                });
                $("#categorys").val(cids);
