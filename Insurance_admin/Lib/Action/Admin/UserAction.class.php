...
class UserAction extends CommonAction {
	...
	function _before_add(){
		...
		$business_source = M('tp_bank_admin.insurance_company','xc_');
        $citylist = $model_city->select();
		$business_source_list = $business_source->select();

        $this->assign('citylist',$citylist); // 模板变量赋值
        $this->assign('business_source_list',$business_source_list); // 模板变量赋值
	}
