...
class StorehouseitemAction extends CommonAction {	
	...
	public function index() {
		...
		$page = $p->show_admin();
		// 当前页数据查询
		$list = $this->storehouse_item_model->field('tp_xieche.xc_storehouse.super_cate,tp_xieche.xc_storehouse_item.*')->where($where)->join('LEFT JOIN tp_xieche.xc_storehouse ON tp_xieche.xc_storehouse_item.s_id = tp_xieche.xc_storehouse.id')->order('tp_xieche.xc_storehouse_item.id ASC')->limit($p->firstRow.','.$p->listRows)->select();
