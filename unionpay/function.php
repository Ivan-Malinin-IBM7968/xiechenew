...
/*
	...
	@time:2013-10-17
*/
function save_txmembercoupon_pay($membercoupon_ids,$trade_no,$buyer_email,$gmt_create,$gmt_payment,$total_fee=0,$trade_status='',$is_return){
	global $mysql;
	$now = time();
	$sql = "INSERT INTO xc_txcouponpay SET membercoupon_ids='".$membercoupon_ids."',trade_no='".$trade_no."',buyer_email='".$buyer_email."',gmt_create='".$gmt_create."',gmt_payment='".$gmt_payment."',total_fee='".$total_fee."',trade_status='".$trade_status."',create_time='".$now."',is_return='".$is_return."'";
	$mysql->query($sql);
}

/*
	@author:ysh
	@function:银联支付测试方法(保存财付通支付信息)
	@time:2014/1/24
*/
function save_unionmembercoupon_pay($membercoupon_ids,$qid,$traceNumber,$orderAmount,$respMsg,$respTime,$is_return){
	global $mysql;
	$now = time();
	$sql = "INSERT INTO xc_chinapaycouponpay SET membercoupon_ids='".$membercoupon_ids."' , qid='{$qid}' , traceNumber='{$traceNumber}' , orderAmount='{$orderAmount}' , respMsg='{$respMsg}' , respTime='{$respTime}' , create_time={$now} , is_return={$is_return} , is_app=2";

	//$sql = "INSERT INTO xc_unioncouponpay SET membercoupon_ids='".$membercoupon_ids."' , qid='".$qid."' , traceNumber='".$traceNumber."' , orderAmount='".$orderAmount."' , respMsg='".$respMsg."' , respTime='".$respTime."' , create_time=".$now." , is_return=".$is_return." , is_app=2";
	
	//$sql = "INSERT INTO  xc_unioncouponpay (membercoupon_ids, qid , traceNumber , orderAmount , respMsg , respTime , create_time , is_return , is_app) VALUES ('{$membercoupon_ids}','{$qid}','{$traceNumber}',{$orderAmount},'{$respMsg}',{$respTime},{$now},{$is_return},2)";
	$mysql->query($sql);
}
...
function coupon_send_sms($membercoupon_id){
	...
	$shop_id = $membercoupon_info['shop_id'];
	$sql_shop = "SELECT shop_name,shop_address FROM xc_shop WHERE id='".$shop_id."' ";
	$shop_info=$mysql->query($sql_shop,'assoc');
