...
if($dopost!='done')
{
    ...
    if(!empty($nid))
    {
        ...
        $totalcc = $mrow['dd'];
        $rrow = $dsql->GetOne("SELECT channelid,usemore,fsid FROM `#@__co_note` WHERE nid='$nid' ");
...
else
{
    ...
    //基本信息主表
    $mainSqlTemplate  = "INSERT INTO `$maintable`(id,typeid,sortrank,flag,ismake,channel,arcrank,click,money,title,shorttitle,color,writer,source,litpic,pubdate,senddate,mid,description,keywords,fsid,shopid)
               VALUES ('@aid@','@typeid@','@sortrank@','@flag@','0','$channelid','$arcrank','0','0','@title@','','','@writer@','@source@','@litpic@','@pubdate@','@senddate@','$adminid','@description@','@keywords@','$fsid','$shopid'); ";
    //生成附加表插入的SQL语句
    $inadd_f = $inadd_v = '';
    ...
    else
    {
        ...
        $pageno++;
        $gourl = "co_export.php?dopost=done&nid=$nid&fsid=$fsid&shopid=$shopid&totalcc=$totalcc&channelid=$channelid&pageno=$pageno";
