<?php
//订单
class PamobileAction extends CommonAction {	
	function __construct() {
		...
		import("ORG.Util.hm");
		$_hmt = new hm("d72ff4cb8351da14591b0a4299e1915c");
	...
	*/
	function index(){
        ...
        redirect(WEB_ROOT.'/mobilecar-carservice.html?from=pingan') ;
        exit ;
        ...
        if ($shop_info){
            ...
            foreach ($shop_info as $k=>$v){
			   ...
			   //$shop_info[$k]['shop_name'] = $v['shop_name'];
	...
	*/
	function order_three(){
		...
		$Paweixin = $this->PaweixinModel->where(array('wx_id'=>$palog['FromUserName'],'type'=>'1'))->find();
		
		if(!$Paweixin){
			$id = $this->PaweixinModel->add(array('wx_id'=>$palog['FromUserName'],'type'=>'1','create_time'=>time()));
			$Paweixin_id = $id;
			$weixin_status = '1';//要验证的状态
		}else{
			$old_weixin = $this->PaweixinModel->where(array('wx_id'=>$palog['FromUserName'],'type'=>'1','status'=>'2'))->find();
		...
		$this->assign('old_weixin',$old_weixin);//微信值
		$this->display();
	}
	...
	*/
	function SendSms(){
		...
		$this->curl_sms($sms);
		*/
		// dingjb 2015-09-29 13:08:52 切换到云通道
		$sms = array(
			'phones' => $mobile,
			'content'=> array(
				$code
	...
	function CheckCode(){
		...
		if($_SESSION['pa_code'] == $code && $_SESSION['pa_mobile'] == $mobile){
			...
			if($Pw_Arr){
				...
				$this->CheckMember($member,$mobile,$Paweixin_id,$code,'1');
			}
			
			$data['userName'] = $this->postdecrypt(urlencode($truename));
			$data['imType'] = urlencode($this->postdecrypt('1'));
			$data['bind'] = urlencode($this->postdecrypt('Y'));
			$data['mobileNo'] = urlencode($this->postdecrypt($mobile));
			$data['imNo'] = urlencode($this->postdecrypt($Pw_Arr['wx_id']));
			//$this->Api_toPA("www.xieche.com.cn/Test-ur",$data);
			//http://dmzstg1.pingan.com.cn:8080/property_insurance/pa18AutoInquiry/mobile/xiechesend.do?
			$pa_result = $this->Api_toPA("http://www.pingan.com/property_insurance/pa18AutoInquiry/mobile/xiechesend.do",$data);
			$testM = D('test');//保险类订单表
			$new['imNo'] = $data['userName'];
			$testM->data($new)->add();
			echo "1";
		}else{
			echo "2";
	...
	*/
	function CheckMember($member,$mobile,$Paweixin_id,$code,$type){
		if($member){
			...
			$member_data['fromstatus'] = '34';//平安WAP
			...
			*/
			// dingjb 2015-09-29 12:00:08 切换到云通讯
			$send_add_user_data = array(
				'phones' => $mobile,
				'content'=> array(
					$mobile,
	...
	function save_order(){
		...
		if($_REQUEST['order_time']){
    		...
    		if(!empty($_POST['order_id'])){
				$url = "http://www.xieche.com.cn/Pamobile/order_list/pa_id/=".$_SESSION['pa_id'];
				header("Location:".$url); 
    			//$this->success('预约提交成功！',__APP__.'/Pamobile/order_list/pa_id/'.$_SESSION['pa_id']);
    		}else {
    			$this->error('预约失败',__APP__.'/Pamobile/index');
    		}
		}
            ...
            if ($Paweixin){
                ...
                if ($number>0 and is_numeric($number)){
                    ...
                    if ($coupon){
						...
						$url = "http://www.xieche.com.cn/txwappay/payRequest.php?membercoupon_id=".$membercoupon_id_str;
						header("Location:".$url); 
						//$this->success('购买成功!','/txwappay/payRequest.php?membercoupon_id='.$membercoupon_id_str);
            	    }else{
						$this->success('操作失败!',__APP__.'/Pamobile/index');
        	        }
                }else{
					$this->success('操作失败!',__APP__.'/Pamobile/index');
    	        }
            }else{
                $this->success('无微信ID!',__APP__.'/Pamobile/index');
            }
        }else{
            $this->success('无微信ID!',__APP__.'/Pamobile/index');
        }
	...
	*/
	function my_coupon() {
		...
		$Padata = $this->PadataModel->where(array('id'=>$_SESSION['pa_id']))->find();
	...
	*/
	function myHome() {
		...
		$url = "http://www.xieche.com.cn/Pamobile-my_account-order-commment-lat-{$FromUserName[Latitude]}-long-{$FromUserName[Longitude]}-pa_id-{$FromUserName[id]}";
		header("Location:".$url); 
	}
