...
class CardaccountAction extends CommonAction {	
	protected $mCardcate;
	...
	public function index() {
		...
		// 计算总数
		$count = $this->mCardinfo->where($where)->join('LEFT JOIN tp_xieche.xc_cardrecord ON tp_xieche.xc_cardinfo.id = tp_xieche.xc_cardrecord.info_id')->count();
		// 导入分页类
		...
		$page = $p->show_admin();
		// 当前页数据查询
		$list = $this->mCardinfo->field('xc_cardinfo.*,xc_cardrecord.type as record_type,xc_cardrecord.receive_person,xc_cardrecord.operator_person,xc_cardrecord.id as record_id,xc_cardrecord.record_num')->where($where)->join('LEFT JOIN tp_xieche.xc_cardrecord ON tp_xieche.xc_cardinfo.id = tp_xieche.xc_cardrecord.info_id')->order('record_id DESC')->limit($p->firstRow.','.$p->listRows)->select();
		if ($list) {
	...
	public function do_account(){
		$id = $this->_post('id');
		if (!$id) {
			$this->error('操作结算失败','',true);
		}
		//更新卡状态	
		$update = array(
			'status'=>6
		);
		$num = $this->mCardlist->where( array('info_id'=>$id,'status'=>4))->count();
		...
		$record_id = $this->mCardrecord->add($recordData);
		
		$this->mCardinfo->where( array('id'=>$id))->save($update);
		//更新卡状态
		$res = $this->mCardlist->where( array('status'=>4))->save($update);
		if($res){
			//程序日志
			$array = array(
					'oid'=>$res,
			...
			$this->success('操作结算成功',U('Storehouse/Cardhouse/index/'),true);
		}else{
			$this->error('操作结算失败','',true);
		}		
	}
