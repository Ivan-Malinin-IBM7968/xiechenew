<?php
class CheckreportAction extends CommonAction {
	...
	*/
    public function index(){
		...
		// 计算总数
		$count = $model_checkreport->where($map)->count();
		...
		// 分页显示输出
		$page = $p->show_admin();
		$code_info = $model_checkreport->where($map)->limit($p->firstRow.','.$p->listRows)->order('id DESC')->select();
		echo $model_checkreport->getLastsql();
