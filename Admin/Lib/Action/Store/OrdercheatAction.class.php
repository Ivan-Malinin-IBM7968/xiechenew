...
class OrdercheatAction extends CommonAction {
    ...
    */
	...
	public function index() {
		...
		$ordercheat_model = D(GROUP_NAME."/ordercheat");
 
        // 计算总数
        $count = $ordercheat_model->where($map)->count();
        // 导入分页类
        ...
        $page = $p->show_admin();
        // 当前页数据查询
        $list = $ordercheat_model->where($map)->order('create_time DESC')->limit($p->firstRow.','.$p->listRows)->select();
		$shop_model = D('Shop');
	...
	*/
	...
	public function selectshop(){
		if($_GET['uid']){
    		...
    		/*if (!$list_membercar){
    		    $model_order = D(GROUP_NAME.'/Ordercheat');
    		    $map_order['uid'] = $uid;
...
*/
    public function addorder() {
		...
		$order_model = D(GROUP_NAME.'/Ordercheat');
    ...
    public function insert(){
		...
		if($_POST['order_date']){
    		...
    		$data = array(
    			...
    			'car_sn'=>$_POST['car_sn'],
    			'remark'=>$_POST['remark'],
    			'operator_id'=>$_SESSION[C('USER_AUTH_KEY_ADMIN')],
	...
	public function edit(){
		...
		$send_add_order_data = '您的车辆'.$vo['licenseplate'].'预定的'.$vo['order_time']['date'].' '.$vo['order_time']['hour'].':'.$vo['order_time']['minute'].'去'.$vo['shop_info']['shop_name'].'['.$vo['shop_info']['shop_address'].']进行'.$servicename.'的服务已预订成功，工时费'.$workhours_sale_str.'，配件费'.$product_sale_str.'，到了4S店后请告知工作人员您是通过携车网预约的，将会有专人为您提供贴心的服务，结算时请出示此短信以享受上述折扣,如有问题请致电携车网'.C('CALL_400');
		...
		if($model->id){
			...
			if ($_POST['order_state'] == 2){
			    ...
			    if ($orderinfo['total_price']>=100){
			        ...
			        $data['operator_id'] = $_SESSION[C('USER_AUTH_KEY_ADMIN')];
	...
	public function update_order_state(){
	    ...
	    if ($id and $order_state){
    	    ...
    	    if($order_state == 1){
			    $this->send_sms($orderinfo,$shop_info,$_REQUEST['send_add_order_data']);
			    $data['status'] = 3;
			    $data['memo'] = '订单确认';
				...
				if($shop_info['shop_mobile']){
					...
					'content'=>"贵店有车型为".$carseries['series_name'].$carmodel['model_name']."牌照为".$orderinfo['licenseplate']."用户于".date('Y-m-t,h:i:s',$orderinfo['order_time'])."预约进店，请给予关照，谢谢",	
					);
				}
	...
	public function addorderlog(){
        ...
        $_POST['operate_id'] = $_SESSION[C('USER_AUTH_KEY_ADMIN')];
        //保存当前数据对象
