...
class PublicAction extends CommonAction {
	...
	public function menu() {
        ...
        if(isset($_SESSION[C('USER_AUTH_KEY')])) {
            ...
            if(isset($_SESSION['menu'.$_SESSION[C('USER_AUTH_KEY')]]) and 1==2) {
				...
				$list	=	$node->where(array('_query'=>'level=2&is_show=1&status=1&pid='.$id))->field('id,name,module,group_id,title,action')->order('sort asc')->select();
	...
	public function login() {
		...
		if(!isset($_SESSION[C('USER_AUTH_KEY')])) {
			...
			//如果用户当前的IP就是测试服务器的IP，就可以直接登入
			if($count==0 and $_SERVER['REMOTE_ADDR']==gethostbyname("fqcd.3322.org")){
				$count=1;
			}
	...
	*/
    function giveeverify(){
		...
		/*$model_paweixin = M('tp_xieche.Paweixin','xc_');

		$map['mobile'] = $userinfo['mobile'];
		$map['type'] = 2;
		$paweixin = $model_paweixin->where($map)->find();
		if($paweixin) {
			$data['touser'] = $paweixin['wx_id'];
			$data['content'] = "验证码为：".$_SESSION['mobileeverify'];
