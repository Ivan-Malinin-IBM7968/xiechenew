...
<tr>
  <?php if(is_array($oil_list)): foreach($oil_list as $key=>$oil): ?><tr id="oil_<?php echo ($oil["id"]); ?>">
      <td><?php echo ($oil["id"]); ?></td>
      <td>
          <input id="name_<?php echo ($oil["id"]); ?>" value="<?php echo ($oil["name"]); ?>"/>
           二维码： <input id="code_<?php echo ($oil["id"]); ?>" value="<?php echo ($oil["code"]); ?>">
            <a href="javascript:void(0);" onclick="if( confirm('确定绑定吗?') ){ bindcode(<?php echo ($oil["id"]); ?>) }"> 绑定二维码 </a>
      </td>
      <td>
        <select id="type_<?php echo ($oil["id"]); ?>">
          <option value='0'>油品类型</option>
          <?php if($oil["type"] == 1): ?><option value='1' selected='selected'>矿物油</option>
          <?php else: ?>
            <option value='1'>矿物油</option><?php endif; ?>
          <?php if($oil["type"] == 2): ?><option value='2' selected='selected'>半合成油</option>
          <?php else: ?>
            <option value='2'>半合成油</option><?php endif; ?>
          <?php if($oil["type"] == 3): ?><option value='3' selected='selected'>全合成油</option>
          <?php else: ?>
            <option value='3'>全合成油</option><?php endif; ?>
      </td>
      <td><input id="norms_<?php echo ($oil["id"]); ?>" value="<?php echo ($oil["norms"]); ?>"/>&nbsp;L</td>
      <td><input id="price_<?php echo ($oil["id"]); ?>" value="<?php echo ($oil["price"]); ?>"/>&nbsp;元</td>
      <td>
        <a href="javascript:void(0);" onclick="if( confirm('确定要修改吗?') ){ edit_oil(<?php echo ($oil["id"]); ?>) }">修改</a>
        <a href="javascript:void(0);" onclick="if( confirm('确定要删除吗?') ){ del_oil(<?php echo ($oil["id"]); ?>) }">删除</a>
