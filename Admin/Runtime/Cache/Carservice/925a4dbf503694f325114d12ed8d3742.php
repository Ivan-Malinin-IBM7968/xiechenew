...
<?php if(!empty($tipss)): ?><div><h3><?php echo ($tipss); ?></h3></div><?php endif; ?>
<div class="xzxm">
<h2 class="xz_title">
    <?php echo ($car_name); ?> <br> 
    价格: <font style="font-size:22px; color:#C00;"><strong id="amount" style="font-size:22px;"></strong> 元</font>
</h2>
<p class="jg">
	
	<?php if((($authId == 1) OR ($authId == 224) OR ($authId == 234) OR ($authId == 367) OR ($authId == 171) OR ($authId == 243) OR ($authId == 299) OR ($authId == 223) OR ($authId == 305) OR ($authId == 309) OR ($authId == 266) OR ($authId == 372) OR ($authId == 269) OR ($authId == 252) OR ($authId == 242) OR ($authId == 336) OR ($authId == 391) OR ($authId == 346)) AND $tbid_show == 1): ?><span id="taobao_price">
		<label for="tb_id"></label>
		<input name="taobao_price" type="text" id="taobao_price" class="item_i" placeholder="输入淘宝卖出金额" data_id="0" value="" />
		</span><?php endif; ?>
        <?php if($tbid_show == 1): ?><span id="taobao_id">
				<label for="tb_id"></label>
				<input name="taobao_id" type="text" id="taobao_id" class="item_i" placeholder="请准确输入第三方ID" data_id="0" value="" />
			</span><?php endif; ?>

	<?php if($order_type == 2): ?>(套餐 : 上门汽车安全检测)<?php endif; ?>
	<?php if($order_type == 6): ?>(套餐 : 上门汽车保养专业服务)<?php endif; ?>
	<?php if($order_type == 7): ?>(套餐 : 府上养车-黄壳机油套餐)<?php endif; ?>
	<?php if($order_type == 8): ?>(套餐 : 府上养车-蓝壳机油套餐)<?php endif; ?>
	<?php if($order_type == 9): ?>(套餐 : 府上养车-灰壳机油套餐)<?php endif; ?>
</p>
<div class="xz_bd">
  <table width="100%" border="0" cellspacing="5" cellpadding="0" class="table">
    <tr>
      ...
      <td width="5%"><input type="checkbox" checked id="check1" class="checkbox" data-price="<?php echo ($item_sets[0]['price']); ?>" onClick="checkClick(this,1)" /></td>
	  <td width="60%" ><form id="form1" name="form1" method="post" action="">
        <label for="item_0"></label>
        <?php if(empty($item_sets)): ?><input data-oil-1-id="0" data-oil-2-id="0" data-oil-1-num="0" data-oil-2-num="0" name="item_0" type="text" id="item_0" class="item_i oil-input" placeholder="请选择机油" data_id="0" value="自备配件（￥0）" />
        <?php else: ?>
        ...
        <div class="item_d" id="item_0_d" style="position:absolute;z-index:1000;width:360px;left:140px;top:34px;background:#FFF;border:1px solid #CCC;border-radius:4px;line-height:30px;display:none;">
          <ul>
          <?php if(is_array($item_sets)): foreach($item_sets as $k=>$set1): ?><li data-oil-1-id="<?php echo ($set1['oil_1']); ?>" data-oil-2-id="<?php echo ($set1['oil_2']); ?>" data-oil-1-num="<?php echo ($set1['oil_1_num']); ?>" data-oil-2-num="<?php echo ($set1['oil_2_num']); ?>" width="309" height="45" class="CheckboxGroup" data-tag="check1" id="CheckboxGroup0_<?php echo ($set1['id']); ?>" data-id="<?php echo ($set1['id']); ?>" data-price="<?php echo ($set1['price']); ?>" style="padding-left:20px;"><?php echo ($set1["name"]); ?>&nbsp;<?php echo ($car_style['norms']); ?><strong>（￥<?php echo ($set1["price"]); ?>）</strong></li><?php endforeach; endif; ?>
		  <?php if(!empty($item_sets_bu)): if(is_array($item_sets_bu)): foreach($item_sets_bu as $k=>$set2): ?><li data-oil-1-id="<?php echo ($set2['oil_1']); ?>" data-oil-2-id="<?php echo ($set2['oil_2']); ?>" data-oil-1-num="<?php echo ($set2['oil_1_num']); ?>" data-oil-2-num="<?php echo ($set2['oil_2_num']); ?>" width="309" height="45" class="CheckboxGroup" data-tag="check1" id="CheckboxGroup0_<?php echo ($set2['id']); ?>" data-id="<?php echo ($set2['id']); ?>" data-price="<?php echo ($set2['price']); ?>" style="padding-left:20px;"><?php echo ($set2["name"]); ?>&nbsp;1L<strong>（￥<?php echo ($set2["price"]); ?>）</strong></li><?php endforeach; endif; endif; ?>
      ...
      <td width="17%" style="color:#e80012;font-size:14px"><span id="item_price_0" class="item_price"><?php echo ($item_sets['0']['price']); ?></span></td>
    </tr>
	<?php if($tips != ''): ?><tr><td width=100></td><td width=60></td><td><font color=red><?php echo ($tips); ?></font></td></tr><?php endif; ?>
    <tr>
      <td height="40">&nbsp;</td>
	   <td><input type="checkbox" checked id="check2"  data-price="<?php echo ($item_set[1][0]['price']); ?>" class="checkbox" onClick="checkClick(this,2)" /></td>
      <td><form id="form2" name="form2" method="post" action="">
        <label for="item_1"></label>
        
        <?php if(empty($item_set[1])): ?><input name="item_1" type="text" id="item_1" class="item_i" placeholder="请选择机油滤清器" data_id="0" value="自备配件（￥0）" />
        <?php else: ?>
        	<input name="item_1" type="text" id="item_1" class="item_i" placeholder="请选择机油滤清器" data_id="<?php echo ($item_set['1']['0']['id']); ?>" value="<?php echo ($item_set['1']['0']['name']); ?>（￥<?php echo ($item_set['1']['0']['price']); ?>）" /><?php endif; ?>
      ...
      <td style="color:#e80012;font-size:14px"><span id="item_price_1" class="item_price"><?php echo ($item_set['1']['0']['price']); ?></span></td>
    </tr>
    <tr>
      <td height="40">&nbsp;</td>
      <td>
        <input type="checkbox" id="check3" <?php if($order_type == 60 or $order_type == 61): ?>checked<?php endif; ?> data-price="<?php echo ($item_set[2][0]['price']); ?>" class="checkbox" onClick="checkClick(this,3)" />
    </td>
      <td><form id="form3" name="form3" method="post" action="">
        <label for="item_2"></label>
          <?php if(empty($item_set[2])): ?><input name="item_2" type="text" id="item_2" class="item_i" placeholder="请选择空气滤清器" data_id="0" value="自备配件（￥0）" />
    	  <?php else: ?>
	        <input name="item_2" type="text" id="item_2" class="item_i" placeholder="请选择空气滤清器" data_id="<?php echo ($item_set['2']['0']['id']); ?>" value="<?php echo ($item_set['2']['0']['name']); ?>（￥<?php echo ($item_set['2']['0']['price']); ?>）" /><?php endif; ?>	
      ...
      <td style="color:#e80012;font-size:14px"><span id="item_price_2" class="item_price"><?php echo ($item_set['2']['0']['price']); ?></span></td>
    </tr>
    <tr>
      <td height="40">&nbsp;</td>
	   <td><input type="checkbox" id="check4" <?php if($order_type == 14 or $order_type == 33 or $order_type == 47 or $order_type == 52 or $order_type == 55 or $order_type == 60 or $order_type == 61 or $order_type == 68): ?>checked<?php endif; ?> data-price="<?php echo ($item_set[3][0]['price']); ?>" class="checkbox" onClick="checkClick(this,4)" /></td>
      <td><form id="form4" name="form4" method="post" action="">
        <label for="kt"></label>
          <?php if(empty($item_set[3])): ?><input name="item_3" type="text" id="item_3" class="item_i" placeholder="请选择空调滤清器" data_id="0" value="自备配件（￥0）" />
          <?php else: ?>
        	<input name="item_3" type="text" id="item_3" class="item_i" placeholder="请选择空调滤清器" data_id="<?php echo ($item_set['3']['0']['id']); ?>" value="<?php echo ($item_set['3']['0']['name']); ?>（￥<?php echo ($item_set['3']['0']['price']); ?>）" /><?php endif; ?>
      ...
      <td style="color:#e80012;font-size:14px"><span id="item_price_3" class="item_price"><?php echo ($item_set['3']['0']['price']); ?></span></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      ...
      <td><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr style="border:none">
          ...
          <td width="50%" align="left">
              ...
              <input type="hidden" name="CheckboxGroup0_res" id="CheckboxGroup0_res" value="<?php echo ($item_sets['0']['id']); ?>">
              <input type="hidden" name="CheckboxGroup1_res" id="CheckboxGroup1_res" value="<?php echo ($item_set['1']['0']['id']); ?>">
...
<script>
    ...
    //var amount = <?php echo ($item_sets['0']['price'] + $item_set['1']['0']['price'] + $item_set['2']['0']['price'] + $item_set['3']['0']['price'] + 99); ?>;
    var amount = <?php echo ($item_sets['0']['price'] + $item_set['1']['0']['price'] + 99); ?>;
