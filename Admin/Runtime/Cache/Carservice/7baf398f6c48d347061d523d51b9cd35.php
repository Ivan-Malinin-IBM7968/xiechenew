...
<tr>
  <?php if(is_array($list)): foreach($list as $key=>$filter): ?><tr id="filter_<?php echo ($filter["id"]); ?>">
      ...
      <td>
          <input id="name_<?php echo ($filter["id"]); ?>" value="<?php echo ($filter["name"]); ?>"  size="40"/>
...
function edit_filter(_item_id){
  ...
  $.ajax({
    url: '__APP__/Carservice/Airconditionfilterconfig/ajax_edit',
    type: 'POST',
...
function del_filter(_item_id){
  $.ajax({
    url: '__APP__/Carservice/Airconditionfilterconfig/ajax_del',
    type: 'POST',
    data:{ id:_item_id},
    dataType: 'json',
    error: function(){
...
function add(){
  window.location.href='__APP__/Carservice/Airconditionfilterconfig/add';
}
...
function bindcode(_item_id){
    var _code=$('#code_'+_item_id).val();
    $.ajax({
          url: '__APP__/Carservice/Airconditionfilterconfig/ajax_bindcode',
          type: 'POST',
