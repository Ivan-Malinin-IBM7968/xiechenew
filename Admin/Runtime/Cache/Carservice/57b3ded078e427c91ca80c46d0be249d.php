...
<tr>
  <?php if(is_array($list)): foreach($list as $key=>$filter): ?><tr id="filter_<?php echo ($filter["id"]); ?>">
      ...
      <td>
          <input id="name_<?php echo ($filter["id"]); ?>" value="<?php echo ($filter["name"]); ?>" size="30"/>
...
function del_filter(_item_id){
  $.ajax({
    url: '__APP__/Carservice/Airfilterconfig/ajax_del',
    type: 'POST',
    data:{ id:_item_id},
    dataType: 'json',
    //timeout: 1000,
...
function add(){
  window.location.href='__APP__/Carservice/Airfilterconfig/add';
}
...
function bindcode(_item_id){
  ...
  $.ajax({
        url: '__APP__/Carservice/Airfilterconfig/ajax_bindcode',
        type: 'POST',
