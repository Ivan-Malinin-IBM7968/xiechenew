...
function save_txmembercoupon_pay($membercoupon_ids,$trade_no,$buyer_email,$gmt_create,$gmt_payment,$total_fee=0,$trade_status='',$is_return){
	global $mysql;
	$now = time();
	$sql = "INSERT INTO xc_txcouponpay SET membercoupon_ids='".$membercoupon_ids."',trade_no='".$trade_no."',buyer_email='".$buyer_email."',gmt_create='".$gmt_create."',gmt_payment='".$gmt_payment."',total_fee='".$total_fee."',trade_status='".$trade_status."',create_time='".$now."',is_return='".$is_return."',is_app=1";
	$mysql->query($sql);
}
...
function update_membercoupon_state($membercoupon_id,$pay_status){
	...
	$now = time();
	$sql = "UPDATE xc_membercoupon SET pay_type=1,is_pay='".$pay_status."',pay_time='".$now."',coupon_code='".$rand_str."' WHERE membercoupon_id='".$membercoupon_id."' LIMIT 1";
	$mysql->query($sql);
}
