...
exit("加载中，请勿刷新。<SCRIPT LANGUAGE=\"JavaScript\">window.location='$url';</SCRIPT><meta http-equiv=\"refresh\" content=\"1; url=$url\">");
}
...
function form_radio($elname,$nowvalue,$d=array('是'=>1,'否'=>0)){
$x="";
if(count($d)<2){
exit('不是数组');
}
...
function pages($align="center",$pageName,$queryString,$total,$ppp,$page,$botton=null,$input_pagenum=null){
	...
	$pages.="<tr><td nowrap align=$align>共 $totalPage 页";
	$m=$page-1;if($m<1) $m=1;if($m>$totalPage) $m=$totalPage;
	$n=$page+1;if($n<1) $n=1;if($n>$totalPage) $n=$totalPage;
	if($page>4) $pages.=" <a href=\"{$queryString}&{$pageName}=1\" class=\"bluelink\">&laquo;首页</a>&nbsp;";
	...
	if($page<$totalPage-5) $pages.="&nbsp;<a href=\"{$queryString}&{$pageName}=$totalPage\" class=\"bluelink\" >末页&raquo;</a> ";
	if(!$botton){
		$pages.=" &nbsp;第<INPUT TYPE=\"text\" NAME=\"$pageName\" VALUE=\"$page\" SIZE=3 id=\"pagenum\">页 </td></tr></form></table>";
	}else{
		$pages.=" &nbsp;第<INPUT TYPE=\"text\" NAME=\"$pageName\" VALUE=\"$page\" SIZE=3 id=\"$input_pagenum\">页 $botton</td></tr></table>";
