...
function update_membercoupon_state($membercoupon_id,$pay_status){
	...
	$now = time();
	$sql = "UPDATE xc_membercoupon SET is_pay='".$pay_status."',pay_time='".$now."',coupon_code='".$rand_str."' WHERE membercoupon_id='".$membercoupon_id."' LIMIT 1";
	$mysql->query($sql);
}

function save_membercoupon_pay($membercoupon_ids,$trade_no,$total_fee=0,$trade_status=''){
	global $mysql;
	$now = time();
	$sql = "INSERT INTO xc_couponpay SET membercoupon_ids='".$membercoupon_ids."',trade_no='".$trade_no."',total_fee='".$total_fee."',trade_status='".$trade_status."',create_time='".$now."'";
	$mysql->query($sql);
}
...
/*
	...
	@time:2013/5/8
*/
function get_insurance_info($bidorder_id) {
	global $mysql;
	//获取保险订单id
	$sql = "SELECT * FROM xc_bidorder WHERE id={$bidorder_id} ";
	$bidorder_info = $mysql->query($sql,assoc);
	
	$insurance_id = $bidorder_info['insurance_id'];
	$shop_id = $bidorder_info['shop_id'];

	//取得店铺名称
	$sql = "SELECT shop_name FROM xc_shop WHERE id={$shop_id} ";
	$shop_name = $mysql->query($sql,1);

	//获取用户填写的信息包含定损金额
	$sql = "SELECT * FROM xc_insurance WHERE id={$insurance_id} ";
	$insurance_info = $mysql->query($sql,"assoc");

	//支付宝要求的订单ID和商品描述
	$insurance_info['bidorder_id'] = $bidorder_id;
	$insurance_info['pay_subject'] = $shop_name.",车辆维修".$insurance_info['loss_price']."元";
...
/*
	...
	@time:2013/5/9
*/
function get_bidorder($bidorder_id) {
	global $mysql;
	$sql = "SELECT * FROM xc_bidorder WHERE id={$bidorder_id} ";
	return $mysql->query($sql,"assoc");
}
...
/*
	...
	@time:2013/5/9
*/
function save_biddingpay($bidorder_id,$trade_no,$total_fee=0,$trade_status='') {
	global $mysql;
	$now = time();
	$sql = "INSERT INTO xc_biddingpay SET bidorder_ids='{$bidorder_id}',trade_no='{$trade_no}',total_fee='{$total_fee}',trade_status='{$trade_status}',create_time='{$now}' ";
	$mysql->query($sql);
}
...
/*
	...
	@time:2013/5/9
*/
function update_bidorder_state($bidorder_id,$pay_status) {
	global $mysql;
	$now = time();
	$sql = "UPDATE xc_bidorder SET pay_status='{$pay_status}',pay_time='{$now}'  WHERE id={$bidorder_id} ";
	$mysql->query($sql);
}
?>